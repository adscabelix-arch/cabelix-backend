...
def build_precio_html(data):
    ...
    base_zonas_map = {1: 75000, 2: 100000, 3: 150000}
    precio_base = base_zonas_map.get(n_zonas, n_zonas * 75000)
    # BHT (Body Hair Transplant): si está activo, suma $30,000 fijos al precio base
    # (Dr. Julio: 1 zona 75->105, 2 zonas 100->130, 3 zonas 150->180)
    if data.get('bht', False):
        precio_base += 30000
    # Factor de escala respecto a la tabla original (calibrada a $75,000 = 1 zona)
    factor = precio_base / 75000

    def _esc(v):
        # Escala y redondea al millar más cercano para montos limpios
        return int(round(v * factor / 1000) * 1000)

    # --- Plan 1: LIQUIDACIÓN (el consultor escribe el monto a mano) ---
    liq_monto  = data.get('p_liq', '')
    liq_limite = data.get('p_liq_limite_txt', '')

    # --- Plan 2: ACCESO INMEDIATO (50% + 50% en h_meses) ---
    h_meses = int(data.get('h_meses', 0) or 0)
    costo_map = {3: 60000, 6: 63000, 9: 67000, 12: 70000}
    hit_tot   = _esc(costo_map.get(h_meses, 0))
    hit_mitad = hit_tot // 2
    hit_mens  = -(-hit_mitad // h_meses) if h_meses else 0

    # --- Plan 3: FINANCIAMIENTO INTERNO (pago inicial + p_fmeses) ---
    fin_ini = int(data.get('p_finicial', 0) or 0)
    fin_mes = int(data.get('p_fmeses', 0) or 0)
    pai_map = {6: 63000, 9: 67000, 12: 70000}
    pai_base = _esc(pai_map.get(fin_mes, 70000))
    tope_fin = _esc(75000)
    inc_max  = _esc(12000)
    ini_pai = pai_base / 2
    if fin_ini >= ini_pai:
        ...
        deficit = (ini_pai - fin_ini) / ini_pai
        inc = round(deficit * inc_max / 1000) * 1000
        fin_tot = min(pai_base + inc, tope_fin)
...
def generar_pdf(data):
    ...
    n_zonas     = len(zonas)
    # Precio de lista con el mapa correcto por zonas (mismo que en build_precio_html)
    base_zonas_map = {1: 75000, 2: 100000, 3: 150000}
    lista_precio = base_zonas_map.get(n_zonas, n_zonas * 75000) if n_zonas else 75000
    # BHT: si está activo, suma $30,000 al precio de lista
    if bht:
        lista_precio += 30000

...
def build_precio_html(data):
    ...
    # Las fórmulas de cálculo se mantienen idénticas a las ya validadas.
    ...
    hit_tot   = costo_map.get(h_meses, 0)
    hit_mitad = hit_tot // 2
    ...
    pai_base = pai_map.get(fin_mes, 70000)
    ini_pai = pai_base / 2
    if fin_ini >= ini_pai:
        fin_tot = pai_base
    else:
        ...
        fin_tot = min(pai_base + inc, 75000)
